...
import { API_URL_CARS, API_URL_CARS_GETALL, options } from '../../constants';
...
class DeleteForm extends Component {
  ...
  componentDidMount() {
    axios.get(API_URL_CARS_GETALL, options).then(res => this.setState({ cars: res.data, id: (res.data.length === 0 ? "" : res.data[0].id), isLoading: false })).catch((error) => {
      console.log(error);
  ...
  handleDeleteClick(e) {
    ...
    axios.delete(API_URL_CARS.concat(this.state.id), options).then(() => axios.get(API_URL_CARS_GETALL, options).then(res => this.setState({ cars: res.data, id: (res.data.length === 0 ? "" : res.data[0].id) })));
  }
