...
import { API_URL_CARMODELS_GETALL, options, API_URL_CARS_CREATE } from '../../constants';
...
class CreateForm extends Component {
    ...
    componentDidMount() {
        axios.get(API_URL_CARMODELS_GETALL, options).then(res => this.setState({ models: res.data, modelid_id: (res.data.length === 0 ? "" : res.data[0].id), isLoading: false })).catch((error) => {
            console.log(error);
    ...
    handleSelectChange(e) {
        this.setState({ modelid_id: e.target.value });
    }
    ...
    handleCreateClick(e) {
        ...
        if(this.state.formValid) {
            ...
            if(this.state.modelid_id)
                jsonBody += ("\"modelid_id\": ".concat(this.state.modelid_id));
            ...
            axios.post(API_URL_CARS_CREATE, jsonBody, options).then(res => this.setState({ createdCar: res.data, isSubmitted: true }));
        }
    }
                    ...
                    <Table responsive>
                        ...
                        <tbody>
                            <tr key="1">
                                ...
                                <td>{this.state.createdCar.modelid_id}</td>
                            </tr>
                        </tbody>
                    </Table>
                    <br/>
